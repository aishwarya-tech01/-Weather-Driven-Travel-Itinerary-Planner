...
def index():
    ...
    if request.method == 'POST':
        ...
        requested_days = int(request.form.get('days', 1)) # Reads dropdown selector option
        ...
        sunny_activities = [
            "🚶 08:30 AM - Guided history walking tour across the old city quarter",
        ...
        # ---- AUTOMATED SIMULATION MODE (Runs immediately without waiting for API keys) ----
        if API_KEY == 'YOUR_ACTUAL_API_KEY_HERE' or not API_KEY:
            is_rainy_profile = any(x in city.lower() for x in ["london", "paris", "rain", "pune"])
            
            temp = 15 if is_rainy_profile else 29
            desc = "Overcast Rain (Simulation Mode)" if is_rainy_profile else "Clear Sunny Skies (Simulation Mode)"
        ...
        try:
            ...
            error_message = "Network connection timeout. Please check your web adapter access settings."
...
def build_itinerary_payload(city, temp, desc, main_condition, days, rainy_acts, sunny_acts):
    ...
    matching icons, and custom packing items into our dashboard view.
    """
    # 🌧️ Rainy/Stormy Custom Styles and Packing Lists
    if any(cond in main_condition for cond in ["rain", "drizzle", "thunderstorm", "snow", "clouds"]):
        icon = "⛈️"
        bg_color = "#0b1329"      # Stormy deep blue-gray page background
        accent_color = "#f43f5e"  # Vivid crimson red border theme
        packing_list = ["☔ Compact Umbrella", "🧥 Windbreaker / Raincoat", "🾾 Waterproof Shoes", "🔋 Power Bank Extra", "🍿 Indoor Card Games"]
    ...
    # ☀️ Sunny/Clear Custom Styles and Packing Lists
        ...
        packing_list = ["🕶️ UV Sunglasses", "🧴 Sunscreen SPF 50+", "🚰 Water Bottle", "🧢 Breathable Hat", "🩳 Lightweight Clothing"]
